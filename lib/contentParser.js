...
const { parseMDX } = require("./utils/mdxParser");
...
const getSinglePage = (folder) => {
  ...
  const filterByDate = publishedPages.filter(
    (page) => {
      // If no date is specified, always include the page
      if (!page.frontmatter.date) {
        return true;
      }
      // Only show posts on or before the current day (end of today in local time)
      const postDate = new Date(page.frontmatter.date);
      const endOfToday = new Date();
      endOfToday.setHours(23, 59, 59, 999);
      return postDate <= endOfToday;
    }
  );
