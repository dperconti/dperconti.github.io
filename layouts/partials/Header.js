...
const Header = () => {
  ...
  useEffect(() => {
    ...
    // Filter to show only specific categories: Fintech, Python, TypeScript, Mental Health
    const allowedCategories = ['Fintech', 'Python', 'TypeScript', 'Mental Health'];
    const filteredCategories = categories.filter(cat => 
      allowedCategories.includes(cat.name)
    );
    
    // Insert filtered categories after "All Posts"
  ...
  return (
    <>
      <header className={`header mt-8 pt-12 pb-3 transition-colors duration-200`}>
